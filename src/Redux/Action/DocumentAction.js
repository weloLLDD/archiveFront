...
export const listDocument = (
  ...
  try {
    ...
    const { data } = await axios.get(
      `https://archivedb.onrender.com/api/documents?page=${page}&pageSize=${pageSize}&type=${type}&year=${year}&searchTerm=${searchTerm}`,
      config
    );
...
export const deleteDocument = (id) => async (dispatch, getState) => {
  try {
    ...
    await axios.delete(`https://archivedb.onrender.com/api/documents/${id}`, config);
...
export const createDocument = (title, description, type, file) => async (
  ...
  try {
    ...
    const { data } = await axios.post(`https://archivedb.onrender.com/api/documents/add`, formData, {
      headers: {
...
export const consultDocument = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`https://archivedb.onrender.com/api/documents/${id}`, config);
